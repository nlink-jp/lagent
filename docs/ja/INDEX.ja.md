...
- [`ADR-0020`](adr/0020-inline-images-declare-their-height.ja.md) —
  ...
  （**Proposed**、未実装。検証パス 3 本を経て供給源の問いを外へ切り出した。
  gem-agent ADR-0089 が反対側の同じ決定）: ADR-0005 は画像がモデルへ届く道を
  決めたが、操作者へ届く道は依然として開いている。`emit` は全ての行の物理行数を
  ...
  **view 層はファイルを開かない。** 決定 4 は sixel が行数を宣言できないことだけに
  乗る。サプライチェーン規律は最初に誤って撤回され、次にエンコーダへ誤って拡張された
  — 一次典拠自身が「API クライアント向けに書かれた」と書いている。`internal/tui` の
  scrollback 会計は両者の共有機構リストに載り、`WithAutoStyle` の危険注記は継承物
  として記録された
- [`ADR-0021`](adr/0021-an-images-bytes-never-become-a-path.ja.md) —
  画像のバイト列は、一度もパスにならずに画面へ届く（**Proposed**、未実装。
  ...
  `prog.Send` に配線される。文字列契約は動かず、sink は対話的 TUI でない入口
  すべてで nil である。画像を描くのは intake が保存し**かつ**記述したときに
  ...
  として名指す: サーバが操作者の画面に絵を置けるようになる。こちらにはまだ実装が
  無い。レーンが向こうにあるので gem-agent が先に行き、決定は両側で同時に採る。
  片側だけが持たないためである
