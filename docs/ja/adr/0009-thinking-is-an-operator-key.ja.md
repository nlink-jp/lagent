...
- 空 completion の不具合は計測した地点では thinking on で起きない。ADR-0007
  ...
  ない。
- ベンチ計測、2026-09-12（`reference/bench.md`）: 6 タスク各 3 反復で、
  thinking on は 18/18 完了、ベースラインは 17/18（1 失敗はガードが止めた
  ループで、空の不具合ではない）。空 completion は on でゼロ、off で 3 回
  （すべて ADR-0007 の再送で回復）。代価は壁時計 252 秒に対し 526 秒、prompt
  357k トークンに対し 463k、reasoning 53 トークンに対し 12,398。編集系が
  最も払い（read-edit 14 秒 → 40 秒、改名 30 秒 → 70 秒）、lookup 系はほぼ
  払わない。**既定は未設定のまま**: 18 中 1 タスクを 2 倍の時間で得るのは
  既定ではなく、消える不具合は別に覆われている。キーは難しいタスク向けに
  操作者のもので、難しいタスクで構成したプロジェクトがこの行を変える計測に
  なる。
