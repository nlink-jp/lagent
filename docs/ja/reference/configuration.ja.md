...
それ以外は通過: 出力無しの exit 0 は呼び出しを無言で通常の承認梯子へ
送る。フックは呼び出しを拒めるが、承認は決してできない。クラッシュ、
タイムアウト（`timeout_sec`、既定 10）、判定ではない出力を伴う exit 0
— 平文、解釈できない JSON、または契約のフィールドも値も持たない JSON —
は、フック名と出力の先頭行を添えた警告をセッションに出して進む。
...
ブロックから受け取ります。これは lagent が起こす全ての子に当てはまり、
`--no-sandbox` も例外ではありません。あのフラグが外すのはカーネルであって、
この規則ではありません（ADR-0017 §2 修正版）。
...
|---|---|
| `--version` | 版数を表示して終了 |
| `-p`, `--prompt` | 単発: このプロンプトを実行して終了。変更を伴うツールは `--allow` に列挙するか `--auto` を付けない限り拒否。資格情報の読取は常に拒否 |
| `--auto` | 自動承認モードで開始: 規則層で Safe の呼び出しは尋ねずに走る |
| `--allow` | この実行で尋ねないツール: 名前または `mcp__server__*` 接頭辞。資格情報の読取は尋ねる |
