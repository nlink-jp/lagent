...
func TestReconnectTouchesOnlyTheNamedServer(t *testing.T) {
	...
	for _, s := range []*stubServer{a, b} {
		if !attachMCPServer(ctx, s, time.Second, reg, &warn, filterOf(t), inv, nil) {
	...
	kept := reconnectMCPServer(ctx, "a", a, func() mcpServer { t.Fatal("a is running; nothing to start"); return nil }, time.Second,
		reg, &warn, filterOf(t, "a/x"), inv, nil)
...
func TestReconnectServerOffClosesItAndOnStartsIt(t *testing.T) {
	...
	ctx := context.Background()
	var warn bytes.Buffer
	attachMCPServer(ctx, a, time.Second, reg, &warn, filterOf(t), inv, nil)
	attachMCPServer(ctx, b, time.Second, reg, &warn, filterOf(t), inv, nil)

	kept := reconnectMCPServer(ctx, "a", a, nil, time.Second,
		reg, &warn, filterOf(t, "a"), inv, nil)
	if kept != nil {
...
func TestReconnectReportsAStaleEntry(t *testing.T) {
	...
	reconnectMCPServer(ctx, "a", a, nil, time.Second,
		reg, &warn, filterOf(t, "a/nope"), inv, nil)
...
func TestReconnectRemovesTheServersOwnNamesNotAPrefix(t *testing.T) {
	...
	t.Run("a neighbour sharing the prefix keeps its tools", func(t *testing.T) {
		...
		var warn bytes.Buffer
		attachMCPServer(ctx, foo, time.Second, reg, &warn, filterOf(t), inv, nil)
		attachMCPServer(ctx, bar, time.Second, reg, &warn, filterOf(t), inv, nil)

		reconnectMCPServer(ctx, "foo", foo, nil, time.Second,
			reg, &warn, filterOf(t, "foo/x"), inv, nil)
	...
	t.Run("a name truncated past its prefix is still removed", func(t *testing.T) {
		...
		var warn bytes.Buffer
		if !attachMCPServer(ctx, srv, time.Second, reg, &warn, filterOf(t), inv, nil) {
		...
		kept := reconnectMCPServer(ctx, long, srv, nil, time.Second,
			reg, &warn, filterOf(t, long+"/function_one"), inv, nil)
		if kept != srv {
